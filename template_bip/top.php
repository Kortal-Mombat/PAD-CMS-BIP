<?php
	if(!isset($_COOKIE['cookieOK']) || $_COOKIE['cookieOK'] != 1)
	{	
	?>
...
<div id="headerWrapper">
	...
	<div id="header">
        ...
        <div id="fontWrapper">
            ...
            if (isset($_SESSION['contr']) && $_SESSION['contr'] == 0)
                $set_contrast = 1;
            else
                $set_contrast = 0;
        ...
        <div id="linksWrapper">
        	<ul>
                <li><a href="http://bip.gov.pl" title="<?= $TXT['but_bip']; ?>" class="bipLink"><img src="/<?php echo $templateDir; ?>/images/logoBIPTop.png" alt="<?= $TXT['but_bip_short']; ?>" /></a></li>
                <?php 
				if (isset($pageInfo['www']) && $pageInfo['www'] != '')
				{
					?>
					<li><a href="<?php echo $pageInfo['www']; ?>" class="topLink icoHome"><?= $TXT['but_start']; ?> podmiotu</a></li>
					<?php
				}
				?>
                
                <li><a href="mapa_strony" class="topLink icoSitemap"><?= $TXT['site_map']; ?></a></li>
            </ul>
        </div>
        ...
        <div id="searchWrapper" role="search">
            <form id="searchForm" name="f_szukaj" method="get" action="index.php">
                ...
                <legend>Szukaj</legend>
                    <label for="kword"  class="hide"><?= $TXT['srch_label']; ?>:</label>
                    <input type="text" id="kword" class="isearch" name="kword" size="24" maxlength="40" value="<?= $TXT['srch_txt']; ?>" onfocus="if (this.value=='<?= $TXT['srch_txt']; ?>') {this.value=''};" onblur="if (this.value=='') {this.value='<?= $TXT['srch_txt']; ?>'};"/>
                    <p><a href="index.php?c=search"><span class="hide">Wyszukiwanie </span>Zaawansowane</a></p>
                    <input type="submit" name="search" value="<?= $TXT['srch_button']; ?>" class="button"/>
                    <div class="clear"></div>
                </fieldset>  
            </form>		
        </div>	    
